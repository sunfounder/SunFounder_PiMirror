...
CODE_DIR = '/home/pi/SunFounder_PiMirror/'
...
class Picture(Frame):
    def __init__(self, parent, *args, **kwargs):
        ...
        self.pictureLb1.pack(side=LEFT, anchor=N)
        self.is_displayed = False
    ...
    def process_status(self, text):
        ...
        if 'show me' in text:
            if 'date' in text or 'time' in text:
                ...
                for key in numbers:
                    if key in text:
                        status = 'NEWS_DETAIL %d'%numbers[key]
...
class Status_Handler(object):
    screensaver_dir = CODE_DIR+'samba_files/screensavers/'
